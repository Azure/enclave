...
The script verifies the standalone compiler checksum, builds and lints all Bicep files, regenerates the five published ARM JSON files, and validates their scopes, metadata, mappings, and every README deployment button. A bounded parser discovers inline, full-reference, collapsed-reference, and shortcut-reference Markdown image links plus direct HTML `<a><img></a>` buttons, independent of alt text. Container-scoped fenced code, CommonMark indented code, inline code spans, and valid HTML comments are removed with offsets preserved before discovery, so non-rendered examples do not count while paragraph and list continuation content remains visible. The parser caps README input at 1 MiB and delimiter nesting at 128, and its direct state-machine scans and delimiter maps avoid rescanning malformed suffixes. Any supported construct containing the official badge or an Azure deployment portal destination is validated; malformed, ambiguous, encoded, or unsupported constructs containing those markers fail closed. The badge and Template URI are then validated structurally and locally without fetching private or public raw GitHub content. Use `-Check` to reproduce the read-only CI drift check.
